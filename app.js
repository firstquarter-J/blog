...
app.post('/hi', (req, res) => {
  try {
    ...
    const hmac = crypto.createHmac('sha256', YOUR_WEBHOOK_KEY).update(JSON.stringify(req.body)).digest('hex');
...
// 라우터를 생성한다. goods.js파일을 라우터로 사용한다.
...
app.get('/detail/:postId', (req, res) => {
  // localhost:5000/detail?goodsId=10의 형식으로 사용, id를 가져온다
